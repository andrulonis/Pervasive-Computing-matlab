classdef mask_detector < matlab.apps.AppBase		%might need to have a warmup on some computers - run
                                                				%it and close the window a few times
    ...
    methods (Access = private)
       ...
       % Function to create the captures folder (if it doesn't exist already)
       function createFolder(app, folderName)
    ...
    methods (Access = public)
        % Construct app
        function app = mask_detector 
            % Create components
            createComponents(app)
            
            % Create the captures folder
            createFolder(app, "captures");
            ...
            while (1)
                try
                    ...
                    % Flips the image. On certain webcams this might not be necessary, comment it out/add it in
                    img = flip(img);
                    ...
                    if faces > 0
                        ...
                        for c = 1:faces
                            ...
                            if (noses == 0 && mouths == 0)
                                ...
                                videoOut = insertObjectAnnotation(videoOut,'rectangle',bbox_face,'WEARING MASK', 'Color', 'green');
                    ...
                    elseif (status == 2)
                        ...
                        % Update label text
                        app.NOFACEFOUNDLabel.Text = ["NOT WEARING MASK PROPERLY" "Cover up Your nose"];
